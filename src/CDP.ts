...
import {DEFAULT_TEQUILA_MIRROR_OPTIONS, DEFAULT_MIRROR_OPTIONS, Mirror, AssetInfo, Token, Asset, MirrorMint, NativeToken, AssetOptions, TerraswapPair} from '@mirror-protocol/mirror.js'
...
export class CDP{
    ...
    async getOnchainReverseSim(shortAmount: Decimal){
        const pair = new TerraswapPair({contractAddress: this.#mirrorClient.assets[this.assetName].pair.contractAddress,lcd: this.#mirrorClient.lcd} )
        const tokenAsset: Asset<Token> = {info: this.assetInfo.info, amount : (shortAmount.times(MICRO_MULTIPLIER)).toFixed(0)}
    ...
    constructCollateralDepositMsg(neededUSTValue: Decimal){
		const collateralAmount = (neededUSTValue.times(MICRO_MULTIPLIER).times(this.minCollateralRatio)).dividedBy(this.collateralPrice)
    ...
    constructBuyAndLPMsg(amount: Decimal, neededUST: Decimal){
		const denomAsset: Asset<NativeToken> = {info: <NativeToken>{native_token: {denom: Denom.USD}}, amount : (neededUST.times(MICRO_MULTIPLIER)).toFixed(0)}
		...
		const coins = new Coins([
			new Coin(denomAsset.info.native_token.denom, denomAsset.amount)
		])

		const msg = [
			new MsgExecuteContract(address, tokenAddress, { // Increase contract allowance
				increase_allowance: {
					spender: pairAddress,
			...
			new MsgExecuteContract(address,pairAddress, { //Buy equal amount of mAsset tokens 
				swap: {
